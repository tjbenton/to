...
// import asyncArray from 'async-array-methods'
...
/// @returns {number}
to.clamp = (value, min = 0, max = Infinity) => {
  if (value > max) {
    return max
  } else if (value < min) {
    return min
...
/// @returns {*}
to.random = function random(min = 0, max = 100) { // issue with tape thats why this isn't a arrow function
  let value
  let length = arguments.length
  let type = to.type(min)
  switch (type) {
    case 'number':
      if (min === max) {
        return min
  ...
  switch (type) {
    ...
    case 'object':
      ...
      break
    default:
      throw new Error('something went wrong with the to.random function')
  }
}
...
to.normalString = (str) => to.string(str).replace(/\r\n|\n/g, '\n')
...
/// @returns {array}
to.keys = (arg) => {
  if (!is.plainObject(arg) && typeof arg !== 'symbol') {
    return arg
  }
...
/// // 0: Jane
/// // 1: Doe
to.entries = (obj, start = 0, end /* = item.length || to.keys(item).length */) => {
  let i = start
...
///   // three -> 'Item three'
/// }
to.objectEntries = (obj, key_name = 'key', index_name = 'i') => {
  let i = 0
  let keys = to.keys(obj)
...
///  }
/// }
to.merge = (a, b, unique = true, flat = true) => { // eslint-disable-line
...
/// @returns {json}
to.json = (arg, spacing = 2) => (is.object(arg) || is.string(arg)) && JSON.stringify(arg, null, spacing)
...
to.flatten = (...args) => {
  ...
  if (type === 'array' || type !== 'object') {
    ...
    throw new Error('to.flatten only accepts arrays *or* objects')
  }
  ...
  return args.reduce((prev, next) => {
    prev = to.extend(prev, flattenObject(next))
...
to.map = (obj, callback, context = null) => {
  if (is.array(obj)) {
...
/// @arg {function} callback
/// @returns {array, object} that was mapped
to.each = (obj, callback, context = null) => {
  if (is.array(obj)) {
...
to.reduce = (obj, callback, initial) => {
  if (is.array(obj)) {
...
to.filter = (obj, callback = Boolean, context = null) => {
  if (is.array(obj)) {
